...
export async function buildFilteredAvailability(
  clubId: string,
  courseId: string,
  dateStr: string,
  playersCount: number,
  ...
  for (const slot of generatedSlots) {
    ...
    let row: CachedAvailabilitySlot;
    if (dbSlot) {
      row = {
        id: dbSlot.id,
    ...
    const fitsParty =
      fullGrid || row.maxPlayers - row.bookedPlayers >= playersCount;
    if (row.status === "open" && fitsParty && new Date(row.datetime) > now) {
      merged.push(row);
    }
  }
